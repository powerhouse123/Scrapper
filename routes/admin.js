...
async function scrapeData(url, page) {
    try {
        ...
        if((outOfStock.includes("This item is currently out of stock")) || (deliveryNotAvaiable.includes("Sold out"))) {
                stock = 'Out of stock';
            } else {
                stock = 'In stock';
            }
            ...
            price = $("span #tp_price_block_total_price_ww> span.a-offscreen").text();
        ...
        let checkDeliveryNotAvailable = $("div.a-section.a-spacing-small.a-text-center > span.a-color-price.a-text-bold");
        if(checkDeliveryNotAvailable) {
            deliveryNotAvaiable = checkDeliveryNotAvailable.text();
        }

        if((outOfStock.includes("This item is currently out of stock")) || (deliveryNotAvaiable.includes("unavailable"))) {
                stock = 'Out of stock';
            } else {
                stock = 'In stock';
            }
        ...
        //let seller = $("#sellerName > span:nth-child(1) > span").text();
...
router.post('/product/new', isAuthenticatedUser, (req,res)=> {
    let {title, price, stock, url, sku, source} = req.body;
